...
function Progress(props: { min: number, max: number, now: number, children?: ComponentChildren }) {
  ...
  return <div class="progress">
    <div class="progress-bar" role="progressbar" style={"width: " + percentage + "%"}
         aria-valuenow={now} aria-valuemin={min} aria-valuemax={max}>{props.children}</div>
  </div>
}
...
export class ReceiveFileModal extends Component<ReceiveFileModalProps, ReceiveFileModalState> {
  ...
  progress() {
    if (this.state.state !== "notstarted") {
      return <Progress min={0} max={this.props.xfer.get_details().size}
  ...
  buttons() {
    switch (this.state.state) {
      case "notstarted":
        return <>
          <Button priority="primary" clickHandler={() => {
            this.accept();
          }}>Accept</Button>
          <Button priority="secondary" clickHandler={() => {
            this.skip();
          }}>Decline</Button>
        </>
      case "started":
        return <>
          <Button priority="danger" clickHandler={() => {
            this.skip();
          }}>Cancel</Button>
        </>
      case "skipped":
    ...
    if (this.state.state != "done")
      return <MyModal title="Send file(s)"
        ...
        <div class="mb-3">
          ...
          <input ref={this.filePickerRef} class="form-control form-control-sm" type="file"
